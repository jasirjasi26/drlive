...
class DoctorsListState extends State<AppointmentHistory1> {
  ...
  Future<void> _selectDate() async {
    ...
    setState(() {
      pastPatientHistory=History().pastPatientData(user_id.value.toString(), from, to, name.text);
    });
  ...
  void initState() {
    super.initState();
    print(user_id.value);
    fetchAppointment = History().activePatientData(user_id.value.toString());
    pastPatientHistory=History().pastPatientData(user_id.value.toString(), from, to, name.text);
  }
                ...
                return SingleChildScrollView(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      int i=index+1;
                      return Container(
                        color: index.floor().isOdd ? Colors.grey[100] : Colors.yellow[100],
                        child: Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceAround,
                            children: [
                              Text(i.toString()),
                              Text(snapshot.data.appointments[index].date.day.toString()+"-"+snapshot.data.appointments[index].date.month.toString()+"-"+snapshot.data.appointments[index].date.year.toString()),
                              SizedBox(),
                              Text(snapshot.data.appointments[index].firstName.toUpperCase()),
                              SizedBox(),
                              Text(snapshot.data.appointments[index].method),
                              Text(snapshot.data.appointments[index].category.name),
                              Text(snapshot.data.appointments[index].fees.toString()),
