...
class ChatRoom extends State<ChatPage> {
  ...
  Future<void> _onPageRefresh() async {
    setState(() {
      fetchAll = NotificationHandler().allChats(user_id.value.toString(),widget.id);
    });
  }
  void onSendMessage() async {
    String token = access_token.value;
    Map data = {
      'from_id': user_id.value,
      'to_id': widget.id,
    ...
    final response =
        await http.post('${AppConfig.BASE_URL}chat/send', body: body, headers: {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        elevation: 0,
        title: Text("Dr."+widget.doctorName.toUpperCase(),style: TextStyle(color: Colors.white),),
            ...
            child: ListView(
              ...
              children: [
                StreamBuilder<Chats>(
                  ...
                  builder: (BuildContext context, AsyncSnapshot<Chats> snapshot) {
                    if (snapshot.hasData) {
                          ...
                          itemBuilder: (context, index) {
                            return Padding(
                              ...
                              padding: EdgeInsets.fromLTRB(
                                data.chats[index].from.id == user_id.value
                                    ? 64.0
                                    : 16.0,
                                4,
                                data.chats[index].from.id == user_id.value
                                      ...
                                      children: [
                                        Text(
                                          ...
                                          style: Theme.of(context)
                                              ...
                                              .bodyText1
                                              .copyWith(
                                                  color: data.chats[index].from.id ==
                                                          user_id.value
                                                      ? Colors.white
                                                      : Colors.black87),
                                        ),
